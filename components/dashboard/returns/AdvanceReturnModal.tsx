...
import { formatReturnStatus, type ReturnStatus } from "@/lib/api/returns";

type AdvanceReturnModalProps = {
  open: boolean;
...
export function AdvanceReturnModal({
  open,
  requestNumber,
  currentStatus,
  nextStatus,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="presentation">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="advance-return-title"
        className="relative w-full max-w-md rounded-xl border border-neutral-200 bg-white shadow-xl"
      >
        <form onSubmit={handleSubmit}>
          <div className="border-b border-neutral-200 px-6 py-4">
            <h2 id="advance-return-title" className="text-lg font-semibold text-neutral-900">
              {actionLabel}
            </h2>
            <p className="mt-1 text-sm text-neutral-500">
              Move <strong>{requestNumber}</strong> from{" "}
              <strong>{formatReturnStatus(currentStatus)}</strong> to{" "}
              <strong>{formatReturnStatus(nextStatus)}</strong>.
            </p>
          </div>
